...
require("f31bf714322ff31a")(require("5b31f1479d155fe0").getBundleURL("bMZQo") + "BarChartStacked.css" + "?" + Date.now());

},{"f31bf714322ff31a":"5SiL5","5b31f1479d155fe0":"ftuBb"}],"5SiL5":[function(require,module,exports) {
"use strict";
...
},{}],"fp4nc":[function(require,module,exports) {
require("14d8df6783fd6f12")(require("e611b0eb300bf510").getBundleURL("bMZQo") + "../commons/palette.css" + "?" + Date.now());

},{"14d8df6783fd6f12":"5SiL5","e611b0eb300bf510":"ftuBb"}],"8NQ4M":[function(require,module,exports) {
require("7e067126f6396e60")(require("21d2733d97c81c57").getBundleURL("bMZQo") + "../commons/tooltip.css" + "?" + Date.now());

},{"7e067126f6396e60":"5SiL5","21d2733d97c81c57":"ftuBb"}],"3N8TJ":[function(require,module,exports) {
...
},{}],"1LDN0":[function(require,module,exports) {
...
class BarChartStacked extends (0, _baseDefault.default) {
    constructor(container, data, options = {}){
        ...
        this.xTimeFormat = options.xTimeFormat || ((d)=>(0, _d3TimeFormat.timeFormat)("%b-%Y")(d));
        ...
        if (data.length) {
            ...
            ].filter((column)=>!this.excludeColumns.includes(column));
        }
    }
    getDimensions() {
    ...
    setupElements() {
        ...
        this.tooltipContainer = (0, _d3Selection.select)(this.container).append("div").attr("class", "gv-tooltip gv-tooltip-bar-stacked");
    ...
    build() {
        ...
        this.g.selectAll(".bar-stacked-group").data((0, _d3Shape.stack)().keys(this.columns).order((0, _d3Shape.stackOrderReverse))(this.data)).join("g").attr("class", "bar-stacked-group").attr("id", ({ key })=>key).attr("fill", ({ key })=>this.scaleColor(key)).selectAll("rect").data((d)=>d).join("rect").attr("class", "bar-stacked-rect").attr("x", (d)=>this.scaleX(d.data[this.xAxisProp])).transition().duration(400).attr("width", this.scaleX.bandwidth()).attr("y", ([y1, y2])=>Math.min(this.scaleY(y1), this.scaleY(isNaN(y2) ? 1 : y2))).attr("height", ([y1, y2])=>Math.abs(this.scaleY(y1) - this.scaleY(isNaN(y2) ? y1 : y2))).attr("cursor", "pointer").selection().on("touchmove", (e)=>e.preventDefault()).on("pointermove", this.onPointerMove.bind(this)).on("pointerout", this.onPointerOut.bind(this));
    ...
    buildLegends() {
        ...
        this.svg.selectAll(".bar-stack-label").remove();
        this.svg.selectAll(".bar-stack-label").data((0, _d3Shape.stack)().keys(this.columns)(this.data)).join((enter)=>{
            const g = enter.append("g").attr("class", "bar-stack-label").attr("fill", ({ key })=>this.scaleColor(key)).attr("transform", (d, i)=>`translate(10, ${i * 14})`);
            g.append("rect").attr("x", positionLegendGroupX).attr("y", (d, i)=>`${this.margin.top + i * (this.wrapLegends ? 24 : 18)}`).attr("rx", 4).attr("class", "bar-stack-label-rect").attr("width", 16).attr("height", 16);
            g.append("text").attr("class", "bar-stacked-legend-text").attr("x", positionLegendLabelX).attr("y", (d, i)=>`${this.margin.top + i * (this.wrapLegends ? 24 : 18) + 14}`).text(({ key })=>key).call(this.wrap, 120, 20);
            return g;
        }, (update)=>update, (exit)=>exit.remove()).on("pointermove", function(_, d) {
            const { key } = d;
            const groups = (0, _d3Selection.selectAll)(".bar-stacked-group");
            groups.filter(({ key: k })=>k !== key).style("opacity", .2);
            groups.filter(({ key: k })=>k === key).style("opacity", 1);
        }).on("pointerout", ()=>{
            (0, _d3Selection.selectAll)(".bar-stacked-group").style("opacity", 1);
        });
        if (this.wrapLegends) this.svg.selectAll(".bar-stack-label").attr("transform", function(d, i) {
            const element = (0, _d3Selection.select)(this)._groups[0][0].getBBox();
            const elementText = (0, _d3Selection.select)(this)._groups[0][0].lastChild;
            if (element.height > 30 && i > 0) elementText.y.baseVal[0].value = elementText.y.baseVal[0].value - 8;
            return `translate(10, ${i * 14})`;
        });
    }
    buildExtraAxis() {
        const extra = this.g.selectAll(".extra-legend").data((0, _d3Shape.stack)().keys(this.extraLegends)(this.data)).enter().append("g").attr("class", "extra-legend").attr("transform", (d, i)=>`translate(0,${this.height + (i + 1) * 28})`);
        extra.append("text").attr("class", "extra-legend-text").attr("text-anchor", "end").attr("x", -60).attr("y", (d, i)=>`${this.margin.top + i * 2 + 14}`).text(({ key })=>key);
        extra.selectAll(".extra-legend-value").data((d)=>d).enter().append("text").attr("class", "extra-legend-value").attr("x", (d)=>this.scaleX(d.data[this.xAxisProp])).attr("y", `${this.margin.top + 14}`).text(([y1, y2])=>(y2 - y1).toLocaleString(undefined, {
    ...
    xAxis(g) {
        ...
        g.call((0, _d3Axis.axisBottom)(this.scaleX).tickFormat((d)=>this.xTimeFormat(d)).tickPadding(6).tickSize(10).tickValues(tickValues));
        // remove baseline
    ...
    setScales() {
        const stacked = (0, _d3Shape.stack)().keys(this.columns)(this.data);
        ...
        this.scaleY = (0, _d3Scale.scaleLinear)().domain([
            0,
            (0, _d3Array.max)(stacked, (d)=>(0, _d3Array.max)(d, (d)=>d[1]))
        ]).nice().range([
            this.height,
            0
        ]);
        this.scaleX = (0, _d3Scale.scaleBand)().domain(this.data.map((d)=>d[this.xAxisProp]).sort((a, b)=>new Date(a) - new Date(b))).paddingInner(0.5).rangeRound([
...
},{"../commons/base":"i3PMv","d3-selection":"gn9gd","d3-array":"1yX2W","d3-time-format":"4FtNS","d3-scale":"UQ8g3","d3-shape":"SqrXv","d3-axis":"2g6gM","d3-transition":"4lorl","./BarChartStacked.css":"4UL6U","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"i3PMv":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _d3Selection = require("d3-selection");
var _d3TimeFormat = require("d3-time-format");
...
},{"d3-selection":"gn9gd","d3-time-format":"4FtNS","../../package.json":"dIKiH","./palette.css":"5jf0d","./tooltip.css":"4d35C","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"gn9gd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{}],"5jf0d":[function(require,module,exports) {
module.exports = require("f52da1ee00aa61f2").getBundleURL("bMZQo") + "../commons/palette.css" + "?" + Date.now();

},{"f52da1ee00aa61f2":"ftuBb"}],"4d35C":[function(require,module,exports) {
module.exports = require("bff9bd83069549e5").getBundleURL("bMZQo") + "../commons/tooltip.css" + "?" + Date.now();

},{"bff9bd83069549e5":"ftuBb"}],"UQ8g3":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"4K6jR":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "default", ()=>function(series) {
...
},{"./none.js":"30Qqt","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"2g6gM":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"./transition/index.js":"h2Srb","./transition/schedule.js":"de74c","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"4UL6U":[function(require,module,exports) {
module.exports = require("7151d68e90ef7d7b").getBundleURL("bMZQo") + "BarChartStacked.css" + "?" + Date.now();

},{"7151d68e90ef7d7b":"ftuBb"}]},["4o6pM","fp4nc","8NQ4M","3N8TJ","1LDN0"], "1LDN0", "parcelRequire3004")

...
require("ff9fb30c44097615")(require("ebabdb01d0daa23").getBundleURL("37Ryj") + "BarChartSplit.css" + "?" + Date.now());

},{"ff9fb30c44097615":"5SiL5","ebabdb01d0daa23":"ftuBb"}],"5SiL5":[function(require,module,exports) {
"use strict";
...
},{}],"iw7Mm":[function(require,module,exports) {
require("d711295c81be53cb")(require("d67803488597b2bf").getBundleURL("37Ryj") + "../commons/palette.css" + "?" + Date.now());

},{"d711295c81be53cb":"5SiL5","d67803488597b2bf":"ftuBb"}],"73APt":[function(require,module,exports) {
require("22a44538f0f953e6")(require("e8280b1419b65c69").getBundleURL("37Ryj") + "../commons/tooltip.css" + "?" + Date.now());

},{"22a44538f0f953e6":"5SiL5","e8280b1419b65c69":"ftuBb"}],"4XgHd":[function(require,module,exports) {
...
},{}],"abVTo":[function(require,module,exports) {
...
class BarChartSplit extends (0, _baseDefault.default) {
    ...
    setupElements() {
        ...
        this.tooltipContainer = (0, _d3Selection.select)(this.container).append("div").attr("class", "gv-tooltip gv-tooltip-bar-chart-small");
        this.g = this.svg.append("g").attr("transform", `translate(${this.margin.left} ${this.margin.top})`);
        this.g.append("g").attr("class", "axis axis-y");
    }
    build() {
        this.setScales();
        const dataGroup = (0, _d3Array.group)(this.data, (d)=>d[this.xAxisProp]);
        ...
        gColumn.append("text").attr("class", "title").text(([key])=>key).attr("y", -21).call(this.wrap, this.width / [
            ...new Set(this.data.map((d)=>d[this.xAxisProp]))
        ].length);
        gColumn.selectAll(".bar-chart-small-groups").data(([, values])=>values).join((enter)=>{
            const g = enter.append("g").attr("class", "bar-chart-small-groups");
            g.append("rect").attr("x", 0).attr("y", (d)=>isNaN(this.scaleY(d[this.yAxisProp])) ? 0 : this.scaleY(d[this.yAxisProp])).attr("width", this.scales.range()[1]).attr("height", this.scaleY.bandwidth()).attr("opacity", ".2").attr("fill", "var(--gv-grey)").attr("class", "bar-chart-small-underlying").attr("display", (d)=>this.scaleY(d[this.yAxisProp]) ? "" : "none");
            g.append("rect").attr("x", 0).attr("y", (d)=>isNaN(this.scaleY(d[this.yAxisProp])) ? 0 : this.scaleY(d[this.yAxisProp])).attr("width", (d)=>this.scales(d[this.countProp])).attr("height", this.scaleY.bandwidth()).attr("fill", (d)=>this.scaleColor(d[this.xAxisProp])).attr("class", "bar-chart-small-overlying").attr("display", (d)=>this.scaleY(d[this.yAxisProp]) ? "" : "none");
            g.append("text").attr("class", "label").style("opacity", ()=>this.showValueOnTooltip ? 0 : 1).style("pointer-events", "none").text((d)=>this.valueIsMoney ? new Intl.NumberFormat("es-ES", {
                    style: "currency",
                    currency: "EUR"
                }).format(d[this.countProp]) : new Intl.NumberFormat("es-ES").format(d[this.countProp])).each((d, i, element)=>{
                const xValue = this.scales(d[this.countProp]);
                const xMax = element[i].getBBox().width + 10;
                if (xValue < xMax && this.moveLabels) (0, _d3Selection.select)(element[i]).attr("x", xValue + 5).attr("fill", "var(--gv-black)");
                else (0, _d3Selection.select)(element[i]).attr("fill", this.moveLabels ? "var(--gv-white)" : "var(--gv-black)").attr("x", 0).attr("dx", 4);
                (0, _d3Selection.select)(element[i]).attr("y", isNaN(this.scaleY(d[this.yAxisProp])) ? 0 : this.scaleY(d[this.yAxisProp]) + this.scaleY.bandwidth() / 2).attr("dy", "0.33em");
            }).attr("display", (d)=>this.scaleY(d[this.yAxisProp]) ? "" : "none");
            return g;
        }, (update)=>update, (exit)=>exit.remove()).on("touchmove", (e)=>e.preventDefault()).on("pointermove", this.onPointerMove.bind(this)).on("pointerout", this.onPointerOut.bind(this)).attr("cursor", this.showValueOnTooltip ? "pointer" : "normal");
    }
    yAxis(g) {
        const tickValues = this.showTickValues ? this.scaleY.domain().filter((d, i)=>this.showTickValues.includes(i)) : this.scaleY.domain();
        g.call((0, _d3Axis.axisLeft)(this.scaleY).tickFormat((d)=>this.yTimeFormat ? this.yTimeFormat(new Date(d)) : d).tickPadding(6).tickSize(10).tickValues(tickValues));
        // remove baseline
    ...
    setScales() {
        ...
        this.scaleY = (0, _d3Scale.scaleBand)().domain(this.sortAxisY ? this.sortAxisY : [
            ...new Set(this.data.map((d)=>d[this.yAxisProp]))
        ].reverse()).range([
            this.height,
            0
        ]).padding(0.4);
        this.scaleColumn = (0, _d3Scale.scaleBand)().domain([
            ...new Set(this.data.map((d)=>d[this.xAxisProp]))
        ]).range([
            0,
            this.width
        ...
        this.scaleXMax = this.groupAxisProps.map((scale)=>{
            return (0, _d3Array.max)(this.data.filter((element)=>scale.includes(element[this.xAxisProp])), (d)=>d[this.countProp]);
        });
        this.scales = (0, _d3Scale.scaleLinear)().range([
            0,
            this.scaleColumn.bandwidth()
        ]).domain([
            0,
            (0, _d3Array.max)(this.scaleXMax)
        ]).nice();
    }
    onPointerMove(event, d) {
        if (!this.showValueOnTooltip) return;
    ...
    onPointerOut() {
        ...
        (0, _d3Selection.selectAll)(".bar-chart-small").transition().duration(200).style("opacity", 1);
    }
    parse(data) {
    ...
    setColorScale() {
        this.scaleColor = (0, _d3Scale.scaleOrdinal)().domain(Array.from(new Set(this.data.map((d)=>d[this.xAxisProp])))).range(this.moveLabels ? this.PALETTE : this.PALETTE.filter((element)=>element !== "var(--gv-color-6)"));
    }
    defaultTooltip(d) {
        return `
      <div class="bar-chart-small-tooltip">
        <h2 class="bar-chart-small-tooltip-title">${d[this.yAxisProp]}</h2>
        <span class="bar-chart-small-tooltip-value">${this.valueIsMoney ? new Intl.NumberFormat("es-ES", {
            style: "currency",
            currency: "EUR"
        }).format(d[this.countProp]) : new Intl.NumberFormat("es-ES").format(d[this.countProp])}</span>
        &nbsp;
      </div>
      `;
    }
...
},{"../commons/base":"i3PMv","d3-selection":"gn9gd","d3-array":"1yX2W","d3-scale":"UQ8g3","d3-axis":"2g6gM","d3-transition":"4lorl","./BarChartSplit.css":"4aeCv","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"i3PMv":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _d3Selection = require("d3-selection");
...
},{"d3-selection":"gn9gd","d3-time-format":"4FtNS","../../package.json":"dIKiH","./palette.css":"fEaTI","./tooltip.css":"jva45","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"gn9gd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{}],"fEaTI":[function(require,module,exports) {
module.exports = require("c51ca5b26eeca579").getBundleURL("37Ryj") + "../commons/palette.css" + "?" + Date.now();

},{"c51ca5b26eeca579":"ftuBb"}],"jva45":[function(require,module,exports) {
module.exports = require("dc8494beea0846ca").getBundleURL("37Ryj") + "../commons/tooltip.css" + "?" + Date.now();

},{"dc8494beea0846ca":"ftuBb"}],"UQ8g3":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"./transition/index.js":"h2Srb","./transition/schedule.js":"de74c","@parcel/transformer-js/src/esmodule-helpers.js":"3H7f3"}],"4aeCv":[function(require,module,exports) {
module.exports = require("c3b67b0c741a65a2").getBundleURL("37Ryj") + "BarChartSplit.css" + "?" + Date.now();

},{"c3b67b0c741a65a2":"ftuBb"}]},["etPzi","iw7Mm","73APt","4XgHd","abVTo"], "abVTo", "parcelRequire3004")
